...
class GraphViewModel<E : Comparable<E>, V: Comparable<V>>(
    ...
    private val _vertices: Map<V, VertexViewModel<V>> = graph.vertices.mapValues { (_, vertex) ->
        VertexViewModel(0.dp, 0.dp, Color.Gray, vertex, showVerticesLabels)
    }

    private val _edges: Map<E, EdgeViewModel<E, V>> = graph.edges.mapValues { (_, edge) ->
        val from = _vertices[edge.from.id]
            ?: throw IllegalStateException("VertexView for ${edge.from.id} not found")
        val to = _vertices[edge.to.id]
