...
class VertexRepository(database: GraphDatabase) {
    private val queries: VerticesQueries = database.verticesQueries

    fun insertVertex(graphId: Long, vertexId: Long, label: String) {
    ...
    fun updateVertexLabelByGraphAndId(graphId: Long, vertexId: Long, newLabel: String) {
        queries.updateVertexLabelByGraphAndId(label = newLabel, graph_id = graphId, vertex_id = vertexId)
    }

    fun deleteVertexByGraphAndId(graphId: Long, vertexId: Long) {
