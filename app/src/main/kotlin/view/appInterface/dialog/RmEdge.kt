            ...
            Button(
                onClick = {
                    ...
                    if (isFromVertexIdEmpty || isToVertexIdEmpty) {
                        if (isFromVertexIdEmpty && isToVertexIdEmpty) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeFromVertexIdAndToVertexIdCannotBeEmpty.value
                            showFromVertexIdError = true
                            showToVertexIdError = true
                        }
                        if (isFromVertexIdEmpty && !isToVertexIdEmpty) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeFromVertexIdCannotBeEmpty.value
                            showFromVertexIdError = true
                        }
                        if (!isFromVertexIdEmpty && isToVertexIdEmpty) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeToVertexIdCannotBeEmpty.value
                            showToVertexIdError = true
                        }
                        return@Button
                    }
                    ...
                    if (isFromVertexIdNotLong || isToVertexIdNotLong) {
                        if (isFromVertexIdNotLong && isToVertexIdNotLong) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeFromVertexIdAndToVertexIdMustBeLong.value
                            showFromVertexIdError = true
                            showToVertexIdError = true
                        }
                        if (isFromVertexIdNotLong && !isToVertexIdNotLong) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeFromVertexIdMustBeLong.value
                            showFromVertexIdError = true
                        }
                        if (!isFromVertexIdNotLong && isToVertexIdNotLong) {
                            errorMessage = LocalizationManager.states.dialogs.rmEdgeToVertexIdMustBeLong.value
                            showToVertexIdError = true
                        }
                        return@Button
                    }
                    onRm("rm edge from:${fromVertexId.text} to:${toVertexId.text}")
                    onDismiss()
                },
